...
open class TenClock : UIControl{
    ...
    open var startIcon: UIImage?
    open var endIcon: UIImage?
    ...
    func imageLayer(image: UIImage) -> CALayer {
        let imageLayer = CALayer()
        imageLayer.backgroundColor = UIColor.clear.cgColor
        imageLayer.bounds.size = CGSize(width: 24, height: 24)
    ...
    func updateHeadTailLayers() {
        ...
        if let startImage = self.startIcon {
            startLayer = imageLayer(image: startImage)
        }else {
            startLayer = tlabel(headText, color: disabledFormattedColor(headTextColor))
        }
        if let endImage = self.endIcon {
            endLayer = imageLayer(image: endImage)
        }else {
            endLayer = tlabel(tailText, color: disabledFormattedColor(tailTextColor))
        }
        
        startLayer.position = topTailLayer.center
        endLayer.position = topHeadLayer.center
        topHeadLayer.addSublayer(startLayer)
        topTailLayer.addSublayer(endLayer)
    }
